...
bool CCardResource::LoadResource()
{
	...
	//桌子扑克
	m_ImageTableTop.LoadResource(IDB_CARD_TABLE_TOP,26,40);
	m_ImageTableLeft.LoadResource(IDB_CARD_TABLE_LEFT,36,32);
	m_ImageTableRight.LoadResource(IDB_CARD_TABLE_RIGHT,36,32);
	m_ImageTableBottom.LoadResource(IDB_CARD_TABLE_BOTTOM,26,40);
	m_ImageTableBottomBig.LoadResource(IDB_CARD_USER_BOTTOM_BIG,37,58);

	//正精图片
	m_ImageTableKingBottom.LoadResource(IDB_CARD_KING_BOTTOM,26,40);
	m_ImageTableKingBottomBig.LoadResource(IDB_CARD_KING_BOTTOM_BIG,37,58);
	m_ImageTableKingTop.LoadResource(IDB_CARD_KING_TOP,26,40);
	m_ImageTableKingLeft.LoadResource(IDB_CARD_KING_LEFT,36,32);
	m_ImageTableKingRight.LoadResource(IDB_CARD_KING_RIGHT,36,32);

	//副精图片
	m_ImageTableViceKingBottom.LoadResource(IDB_CARD_VICE_KING_BOTTOM,26,40);
	m_ImageTableViceKingBottomBig.LoadResource(IDB_CARD_VICE_KING_BOTTOM_BIG,37,58);
	m_ImageTableViceKingTop.LoadResource(IDB_CARD_VICE_KING_TOP,26,40);
	m_ImageTableViceKingLeft.LoadResource(IDB_CARD_VICE_KING_LEFT,36,32);
	m_ImageTableViceKingRight.LoadResource(IDB_CARD_VICE_KING_RIGHT,36,32);
	...
	//精牌的弟弟
	m_ImageKingTop.LoadResource(IDB_KING_TOP, 26, 40);
	m_ImageKingBottom.LoadResource(IDB_KING_BOTTOM, 26, 40);
	m_ImageKingLeft.LoadResource(IDB_KING_LEFT, 28, 25);
	m_ImageKingRight.LoadResource(IDB_KING_RIGHT, 28, 25);
	...
	case Direction_East:	//东向
		{
			...
			if ((m_wFullCount-m_wMinusHeadCount-m_wMinusLastCount)>0)
			{
				...
				if (m_wMinusHeadCount%2!=0)
				{
					...
					nYPos=m_ControlPoint.y+wHeapIndex*18+9;
				...
				for (WORD i=wDoubleHeap;i<wFinallyIndex;i++)
				{
					...
					if(m_wKingPos!=-1 &&i==m_wKingPos)
					{
						...
						int nYPos=m_ControlPoint.y+(m_wKingPos)*18;
				...
				{
					nXPos=m_ControlPoint.x;
					nYPos=m_ControlPoint.y+wFinallyIndex*18+9;
					g_CardResource.m_ImageHeapSingleV.AlphaDrawImage(pDC,nXPos,nYPos,RGB(255,0,255));
				}
				//绘制精牌
				...
				{
					int nXPos=m_ControlPoint.x;
					int nYPos=m_ControlPoint.y+(m_wKingPos)*18+2;
					g_CardResource.m_ImageKingRight.DrawCardItem(pDC, m_cbKingCardData, nXPos, nYPos);
				}
			}	
			else
	...
	case Direction_South:	//南向
		{
			...
			if ((m_wFullCount-m_wMinusHeadCount-m_wMinusLastCount)>0)
			{
				...
				if (m_wMinusLastCount%2!=0)
				{
					...
					nXPos=m_ControlPoint.x+wHeapIndex*26;
				...
				{
					nYPos=m_ControlPoint.y+9;
					nXPos=m_ControlPoint.x+wFinallyIndex*26;
					g_CardResource.m_ImageHeapSingleH.AlphaDrawImage(pDC,nXPos,nYPos,RGB(255,0,255));
				}
			}
			//绘制精牌
			if(m_wKingPos!=-1)
			{
				...
				int nXPos=m_ControlPoint.x+(HEAP_FULL_COUNT/2-m_wKingPos-1)*26;
	...
	/*				if((m_wKingPos!=-1)&&(m_wKingPos==i))
					{
						bHas=true;
						int nXPos=m_ControlPoint.x;
						int nYPos=m_ControlPoint.y+(HEAP_FULL_COUNT/2-m_wKingPos-1)*18;
	...
	case Direction_North:	//北向
		{
			...
			if ((m_wFullCount-m_wMinusHeadCount-m_wMinusLastCount)>0)
			{
				...
				{
					int nYPos=m_ControlPoint.y;
					int nXPos=m_ControlPoint.x+(m_wKingPos)*26;
	...
	case Direction_East:	//东向
		{
			...
			if (m_wCardCount==4)
			{
				nXScreenPos=m_ControlPoint.x+nItemWidth;
				nYScreenPos=m_ControlPoint.y+nItemHeight;
				g_CardResource.m_ImageTableRight.DrawCardItem(pDC,GetCardData(3),nXScreenPos,nYScreenPos-10);
			}
...
BYTE CWeaveCard::GetCardData(WORD wIndex)
{
	...
	return ((m_bDisplayItem==1)||(wIndex==1)||(m_bDisplayItem==2&&wIndex==3))?cbCardData:0;
}
	...
	case Direction_East:	//东向
		{
			...
			if (m_wCardCount>0)
			{
				...
				{
					nXPos=m_ControlPoint.x;
					nYPos=m_ControlPoint.y+i*20+40;
					g_CardResource.m_ImageUserRight.AlphaDrawImage(pDC,nXPos,nYPos,RGB(255,0,255));
				}
			}
	...
	case Direction_West:	//西向
		{
			...
			if (m_wCardCount>0)
			{
				...
				{
					nXPos=m_ControlPoint.x;
					nYPos=m_ControlPoint.y-(m_wCardCount-i-1)*20-92;
	...
	case Direction_East:	//东向
		{
			...
			for (WORD i=0;i<m_wCardCount;i++)
			{
				int nXPos=m_ControlPoint.x+(i/8)*36;
	...
	case Direction_South:	//南向
		{
			...
			for (WORD i=0;i<m_wCardCount;i++)
			{
				int nXPos=m_ControlPoint.x-(i%8)*25;
				int nYPos=m_ControlPoint.y+(i/8)*30;
	...
	case Direction_West:	//西向
		{
			...
			for (WORD i=0;i<m_wCardCount;i++)
			{
				int nXPos=m_ControlPoint.x-((m_wCardCount-1-i)/8)*36;
	...
	case Direction_North:	//北向
		{
			...
			for (WORD i=0;i<m_wCardCount;i++)
			{
				int nXPos=m_ControlPoint.x+((m_wCardCount-1-i)%8)*25;
				int nYPos=m_ControlPoint.y-((m_wCardCount-1-i)/8)*30-11;
	...
	case Direction_East:	//东向
		{
			...
			int nXPos=m_ControlPoint.x+((m_wCardCount-1)/8)*38+5;
	...
	case Direction_South:	//南向
		{
			...
			int nXPos=m_ControlPoint.x-((m_wCardCount-1)%8)*25;
			int nYPos=m_ControlPoint.y+((m_wCardCount-1)/8)*35-8;
	...
	case Direction_North:	//北向
		{
			...
			int nXPos=m_ControlPoint.x+((m_wCardCount-1)%8)*25;
			int nYPos=m_ControlPoint.y+((-m_wCardCount+1)/8)*35-21;
			return CPoint(nXPos,nYPos);
		}
	}
	return CPoint(0,0);
	...
	case Direction_South:	//南向
		{
			...
			for (WORD i=0;i<m_wCardCount;i++)
			{
				...
				int nXPos=m_ControlPoint.x-(m_wCardCount-i)*37;
				BYTE cbCardData = m_cbCardData[i];
				BYTE cbCardIndex=SwitchToCardIndex(cbCardData);
				if(cbCardIndex == m_cbKingCardIndex[0])
				{
					g_CardResource.m_ImageTableKingBottomBig.DrawCardItem(pDC,cbCardData,nXPos,nYPos);

				}
				else if(cbCardIndex==m_cbKingCardIndex[1])
				{
					g_CardResource.m_ImageTableViceKingBottomBig.DrawCardItem(pDC,cbCardData,nXPos,nYPos);

				}
				else
				{

					g_CardResource.m_ImageTableBottomBig.DrawCardItem(pDC,cbCardData,nXPos,nYPos);
				}
			}
