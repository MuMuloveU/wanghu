...
bool CGameClientDlg::OnSubOutCard(const void * pBuffer, WORD wDataSize)
{
	...
	//变幻扑克
	m_GameLogic.MagicCardData(pOutCard->cbCardData,pOutCard->cbCardCount,m_cbMagicCardData);

	BYTE cbStarLevel;
	//获取牌型
	BYTE cbTurnCardType=m_GameLogic.GetCardType(m_cbMagicCardData,pOutCard->cbCardCount,cbStarLevel);	//炸弹判断
	//播放声音
...
bool CGameClientDlg::VerdictOutCard()
{
	...
	BYTE cbShootCount=(BYTE)m_GameClientView.m_HandCardControl[2].GetShootCard(cbCardData,CountArray(cbCardData));
	
	//变幻扑克
	BYTE cbMagicCardData[MAX_COUNT];
	m_GameLogic.MagicCardData(cbCardData,cbShootCount,cbMagicCardData);
	...
	if (cbShootCount>0L)
	{
		...
		//分析类型
		BYTE bCardType=m_GameLogic.GetCardType(cbMagicCardData,cbShootCount,cbStarLevel);
