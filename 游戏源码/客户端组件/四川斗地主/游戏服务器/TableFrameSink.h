...
protected:
	...
	WORD							m_wOutBombCount;					//炸弹数目
	WORD							m_wFirstUser;						//首叫用户
	WORD							m_wBankerUser;						//庄家用户
	WORD							m_wCurrentUser;						//当前玩家
	BYTE							m_bMingCard;						//明牌扑克
	BYTE							m_bCurrentState;					//当前状态
	BYTE							m_bCallScoreTimes[GAME_PLAYER];		//叫分次数
	BYTE							m_bCallScoreTag[GAME_PLAYER];		//叫分判断
	BYTE							m_bScoreTimes[GAME_PLAYER];			//分数倍数
	BYTE							m_bCallBanker[GAME_PLAYER];			//叫庄信息
	BYTE							m_bHaveSendCard;					//发牌判断
	BYTE							m_bBackCatchTag;					//闷抓标志
	BYTE							m_bLookCardTag;						//看牌标志
