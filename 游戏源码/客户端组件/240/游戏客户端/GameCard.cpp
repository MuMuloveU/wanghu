...
//初始化函数
bool CGameCard::InitGameCard(CWnd * pHandleWnd)
{
...
//设置扑克参数
void CGameCard::SetCardParameter(int iHSpace, int iVSpace, int iUpSpace)
{
...
//获取大小函数
bool CGameCard::GetCardSize(CSize & Size)
{
	if (m_bCardCount>0)
	{
		if (m_bHorzLook==true)
...
//设置基准点函数
void CGameCard::SetBasePoint(CPoint & BasePoint, XShowMode XShowMode, YShowMode YShowMode)
{
...
//设置显示扑克
void CGameCard::SetShowCard(bool bShow)
{
	m_bShowCard=bShow;
	Invalidate(FALSE);
	return;
...
//设置使用鼠标
void CGameCard::SetUseMouse(bool bUserMouse)
{
...
//设置显示方式
void CGameCard::SetLookMode(bool bHorzLook)
{
...
//设置扑克
bool CGameCard::SetCard(BYTE bCardList[], BYTE bUpList[], BYTE bCardCount)
{
...
//获取升起的扑克
BYTE CGameCard::GetUpCard(BYTE bCardList[])
{
	BYTE bUpCount=0;
...
//获取扑克信息
BYTE CGameCard::GetCard(BYTE bCardList[], BYTE bUpList[])
{
...
//鼠标消息
void CGameCard::OnLButtonDown(UINT nFlags, CPoint Point)
{
...
//光标消息
BOOL CGameCard::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT message)
{
	if (m_bUseMouse==true)
	{
		HINSTANCE hInstance = ::AfxGetInstanceHandle();
