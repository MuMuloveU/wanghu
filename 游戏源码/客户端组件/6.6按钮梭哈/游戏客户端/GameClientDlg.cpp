	...
	case GS_PLAYING:	//游戏状态
		{
			...
			if ((IsLookonMode()==false)&&(pStatusPlay->wCurrentUser==GetMeChairID()))
			{
				...
				//禁止按钮
				m_GameClientView.m_btShowHand.EnableWindow((m_bShowHand==false)&&(wCardCount>=3&&lLeaveScore>0)?TRUE:FALSE);
				m_GameClientView.m_btFollow.EnableWindow(((m_bShowHand==true)||(lLeaveScore>=0L)||(m_lTurnBasicGold==0L))?TRUE:FALSE);
...
bool CGameClientDlg::OnSubAddGold(const void * pBuffer, WORD wDataSize)
{
	...
	LONG lLeaveScore=m_lTurnMaxGold-m_lTurnBasicGold-m_GameClientView.m_GoldView[5].GetGold();
	WORD wCardCount=m_GameClientView.m_CardControl[2].GetCardData(NULL,0);
	if ( wCardCount >= 3 && lLeaveScore <= 0 ) PlayGameSound(AfxGetInstanceHandle(),TEXT("SHOW_HAND"));
	else if (m_bPlayStatus[pAddGold->wCurrentUser]==TRUE) PlayGameSound(AfxGetInstanceHandle(),TEXT("ADD_SCORE"));

	if( wCardCount >= 3 && lLeaveScore <= 0 && pAddGold->wLastChairID != wMeChairID )
...
LRESULT CGameClientDlg::OnSendCardFinish(WPARAM wParam, LPARAM lParam)
{
	...
	if ((IsLookonMode()==false)&&(m_wCurrentUser==GetMeChairID()))
	{
		...
		//禁止按钮
		WORD wCardCount=m_GameClientView.m_CardControl[2].GetCardData(NULL,0);
		m_GameClientView.m_btShowHand.EnableWindow(((wCardCount>=3)&&(m_bShowHand==false))?TRUE:FALSE);
		m_GameClientView.m_btFollow.EnableWindow(((m_bShowHand==true)||(lLeaveScore>=0L)||(m_lTurnBasicGold==0L))?TRUE:FALSE);
