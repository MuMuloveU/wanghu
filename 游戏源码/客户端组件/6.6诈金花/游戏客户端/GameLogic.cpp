...
BYTE CGameLogic::CompareCard(BYTE cbFirstData[], BYTE cbNextData[], BYTE cbCardCount)
{
	...
	//类型判断
	if (cbFirstType!=cbNextType) return (BYTE)(cbFirstType>cbNextType);
	...
	case CT_JIN_HUA:		//金花
		{
			...
			for (BYTE i=0;i<cbCardCount;i++)
			{
				...
				if (cbFirstValue!=cbNextValue) return (BYTE)(cbFirstValue>cbNextValue);
			}
			return DRAW;
		}
	case CT_SHUN_ZI:		//顺子
	...
	case CT_DOUBLE:			//对子
		{
			...
			if (bNextDouble!=bFirstDouble)return (BYTE)(bFirstDouble>bNextDouble);
			if (bNextSingle!=bFirstSingle)return (BYTE)(bFirstSingle>bNextSingle);
			return DRAW;
		}
	}
