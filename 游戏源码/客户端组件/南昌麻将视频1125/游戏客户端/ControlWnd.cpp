...
void CControlWnd::SetControlInfo(BYTE cbCenterCard, WORD wActionMask, tagGangCardResult & GangCardResult)
{
	...
	for (BYTE i=0;i<CountArray(wItemKind);i++) 
		if ((m_wActionMask&wItemKind[i])!=0) 
			m_cbItemCount++;
...
void CControlWnd::RectifyControl()
{
	...
	m_btChiHu.SetWindowPos(NULL,rcRect.Width()-rcButton.Width()*3,nYPos,0,0,SWP_NOZORDER|SWP_NOSIZE);
...
void CControlWnd::OnPaint()
{
		...
		if ((m_wActionMask&wItemKind[i])!=0)
		{
			if(i<4)
			{
				...
				for (BYTE j=0;j<3;j++)
				{
					...
					g_CardResource.m_ImageTableBottom.DrawCardItem(&BufferDC,cbCardData,j*25+12,nYPos+5);
				}
			}

			if((m_wActionMask&wItemKind[i])&WIK_DNBL)
			{
				BYTE cbCardData[]={m_cbCenterCard,m_cbCenterCard+1,m_cbCenterCard+3};
				//绘画扑克
				for (BYTE j=0;j<3;j++)
				{
					g_CardResource.m_ImageTableBottom.DrawCardItem(&BufferDC,cbCardData[j],j*25+12,nYPos+5);
				}
			}


			if((m_wActionMask&wItemKind[i])&WIK_DNBC)
			{
				BYTE cbCardData[]={m_cbCenterCard-1,m_cbCenterCard,m_cbCenterCard+2};
				//绘画扑克
				for (BYTE j=0;j<3;j++)
				{
					g_CardResource.m_ImageTableBottom.DrawCardItem(&BufferDC,cbCardData[j],j*25+12,nYPos+5);
			...
			if((m_wActionMask&wItemKind[i])&WIK_DNBR)
			{
			...
			if((m_wActionMask&wItemKind[i])&WIK_DXBL)
			{
			...
			if((m_wActionMask&wItemKind[i])&WIK_DXBC)
			{
			...
			if((m_wActionMask&wItemKind[i])&WIK_DXBR)
			{
			...
			if ((m_wActionMask&wItemKind[i])&WIK_PENG)
	...
	for (BYTE i=0;i<CountArray(m_cbGangCard);i++)
	{
		if (m_cbGangCard[i]!=0)
		{
			...
			for (BYTE j=0;j<4;j++)
			{
				g_CardResource.m_ImageTableBottom.DrawCardItem(&BufferDC,m_cbGangCard[i],j*25+12,nYPos+5);
			}
...
void CControlWnd::OnLButtonDown(UINT nFlags, CPoint Point)
{
	...
	if (m_cbCurrentItem!=0xFF)
	{
		...
		for (BYTE i=0;i<CountArray(wItemKind);i++)
		{
			if (((m_wActionMask&wItemKind[i])!=0)&&(m_cbCurrentItem==cbIndex++))
			{
				AfxGetMainWnd()->PostMessage(IDM_CARD_OPERATE,wItemKind[i],m_cbCenterCard);
				return;
			}
		}
