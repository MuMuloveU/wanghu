...
public:	
	...
	//是否BlackJack
	bool IsBlackJack(const BYTE bCardBuffer[],BYTE bCardCount);
	//能否分牌
	bool CanCutCard(const BYTE bFirstCard, const BYTE bLastCard);
	
	//是否为777
	bool IsSevensCard(const BYTE bCardBuffer[],BYTE bCardCunt);
	//是否为N小龙
	BYTE IsNDragon(const BYTE bCardBuffer[],BYTE bCardCount);
