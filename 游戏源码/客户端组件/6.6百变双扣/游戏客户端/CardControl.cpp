...
//设置弹起扑克
DWORD CCardControl::SetShootCard(const BYTE cbCardData[], DWORD dwCardCount)
{
	...
	for (DWORD i=0;i<dwCardCount;i++)
	{
		for (INT_PTR j=0;j<m_CardDataItem.GetCount();j++)
		{
			...
			if ((pCardItem->bShoot==false)&&(pCardItem->bCardData==cbCardData[i])) 
			{
	...
	if (m_bHorizontal==true)
	{
		...
		if (bCurrentCard==false)
		{
			while (dwCardIndex>0)
			{
				...
				if ((bCardShoot==false)&&(MousePoint.y>m_CardSize.cy)) break;
...
void CCardControl::OnLButtonDblClk(UINT nFlags, CPoint Point)
{
	...
	//获取数值
	BYTE cbCardValue=GetCardValue(pCardItem->bCardData);

	//弹起扑克
	BYTE cbShootCardData[MAX_COUNT];
	ZeroMemory(cbShootCardData,sizeof(cbShootCardData));
	...
	{
		pCardItem=&m_CardDataItem[i];
		if (GetCardValue(pCardItem->bCardData)==cbCardValue)
		{
			cbShootCardData[cbSameCount++]=pCardItem->bCardData;
		}
	}
	...
	for (DWORD i=0;i<dwCardCount;i++)
	{
		...
		{
			cbShootCardData[cbSameCount+cbShootCount]=pCardItem->bCardData;
			cbShootCount++;
		}
	}
	//完成处理
	if (cbSameCount>=1)
	{
		SetShootCard(cbShootCardData,cbSameCount+cbShootCount);
		m_dwCurrentIndex=0xFFFFFFFF;	
