...
DWORD CCardControl::GetShootCard(BYTE bCardData[], DWORD dwMaxCount)
{
	...
	for (DWORD i=0;i<dwCardCount;i++)
	{
		...
		pCardItem=&m_CardDataItem[i];
		///保皇游戏中    by zqh 
		/*if (m_bCurrentCard!=m_GameLogic.GetCardLogicValue(pCardItem->bCardData))
			if ( m_bCurrentCard!=16 || m_bCurrentCard!=17)
...
void CCardControl::RectifyControl()
{
	...
	for (DWORD i=0;i<dwCardCount;i++)
	{
		...
		if (m_bHorizontal==true)
		{
			...
			///保皇游戏中    by zqh 
			if (!m_bSetShootCard && m_bCurrentCard!=m_GameLogic.GetCardLogicValue(pCardItem->bCardData))
				if ( m_bCurrentCard < 15)
					pCardItem->bShoot = false;
			if (m_bCtrl)
			{
				if (m_bCurrentCard == m_GameLogic.GetCardLogicValue(pCardItem->bCardData))
...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	///保皇游戏中    by zqh 
	m_bCurrentCard = m_GameLogic.GetCardLogicValue(pCardItem->bCardData);
