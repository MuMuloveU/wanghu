...
void CChessManual::DoDataExchange(CDataExchange * pDX)
{
	...
	DDX_Control(pDX, IDC_RELOAD, m_btReLoad);
	DDX_Control(pDX, IDC_FIRST, m_btFirst);
	DDX_Control(pDX, IDC_BEFORE, m_btBefore);
...
void CChessManual::UpdateManualList()
{
	...
	//变量定义
	LPCTSTR pszChessDes[]={TEXT("将"),TEXT("士"),TEXT("象"),TEXT("马"),TEXT("车"),TEXT("炮"),TEXT("兵")};
	...
	for (INT_PTR i=0;i<m_ChessManualInfo.GetCount();i++)
	{
		...
		_snprintf(szManualText,sizeof(szManualText),TEXT("%s%s [ %ld，%ld ]-[ %ld，%ld ]"),
			((i%2)==0)?TEXT("红"):TEXT("黑"),pszChessDes[pChessManual->cbSourceChess-1],
