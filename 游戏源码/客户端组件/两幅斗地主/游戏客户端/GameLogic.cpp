...
bool CGameLogic::SearchOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult)
{
	...
	case CT_THREE_LINE_TAKE_TWO:	//三带一对
		{
			...
			if (cbTurnOutType==CT_THREE_LINE_TAKE_TWO) cbTurnLineCount=cbTurnCardCount/5;
	...
	if ((cbCardCount>=4)&&(cbTurnOutType!=CT_MISSILE_CARD))
	{
		...
		//搜索炸弹
		for ( BYTE cbBombLen = 8; 3 <= cbBombLen; --cbBombLen )
		{
			for (BYTE i=cbBombLen;i<cbCardCount;i++)
			{
				//获取数值
				BYTE cbHandLogicValue=GetCardLogicValue(cbCardData[cbCardCount-i-1]);
				...
				for (BYTE j=1;j<8 && j<cbCardCount;j++)
				{
					if (GetCardLogicValue(cbCardData[cbCardCount+j-i-1])!=cbTempLogicValue) break;
				}
				if (j<4 || cbTurnOutType==CT_BOMB_CARD && j<cbTurnCardCount) continue;

				//构造判断
				if ( j <= cbTurnCardCount && cbHandLogicValue <= cbLogicValue ) continue;

				//设置结果
				OutCardResult.cbCardCount=j;
				CopyMemory( OutCardResult.cbResultCard, cbCardData + cbCardCount-i-1, j );
	...
	if ((cbCardCount>=4)&&(cbCardData[0]==0x4F)&&(cbCardData[3]==0x4E))
	{
		...
		OutCardResult.cbResultCard[2]=cbCardData[2];
		OutCardResult.cbResultCard[3]=cbCardData[3];
