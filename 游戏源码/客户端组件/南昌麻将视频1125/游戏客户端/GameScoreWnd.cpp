...
void CGameScoreWnd::OnPaint()
{
	...
	//绘画扑克
	g_CardResource.m_ImageUserBottom.DrawCardItem(&BufferDC,m_GameScoreInfo.cbChiHuCard,105,23);
	...
	for (int i=0;i<GAME_PLAYER;i++)
	{
		...
		if (m_GameScoreInfo.wCardType[i]!=0)
			BufferDC.TextOut(180,140+i*30,TEXT("胡牌"),4);
		else if (m_GameScoreInfo.wProvideUser==i)
			BufferDC.TextOut(180,140+i*30,TEXT("放炮"),4);

		//其他信息
		if (m_GameScoreInfo.wBankerUser==i) 
			BufferDC.TextOut(228,140+i*30,TEXT("是"),2);
	}
	////游戏税收
	//rcDraw.left=25;
	//rcDraw.right=125;
