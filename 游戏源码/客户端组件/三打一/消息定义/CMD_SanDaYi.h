...
#ifdef VIDEO_GAME
	#define KIND_ID						503									//游戏 I D
#else
	#define KIND_ID						113									//游戏 I D
#endif

#define GAME_PLAYER					4									//游戏人数
#define GAME_NAME					TEXT("三打一")					//游戏名字
...
{
	WORD							wLandUser;							//坑主玩家
	WORD							bOutBombCount;						//炸弹倍数
	BYTE							bMaxBombCount[4];					//炸弹倍数
	BYTE							bOutBombCount2[4];						//炸弹倍数
	LONG							lBaseScore;							//基础积分
	BYTE							bLandScore;							//地主分数
	WORD							wLastOutUser;						//出牌的人
	WORD				 			wCurrentUser;						//当前玩家
	BYTE							bBackCard[8];						//底牌扑克
	BYTE							bCardData[4][33];					//手上扑克
