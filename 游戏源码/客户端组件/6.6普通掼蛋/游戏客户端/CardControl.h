...
#define IDM_CHANGE_POSITION			(WM_USER+502)						//改变位置
...
protected:
	...
	bool							m_bChange;							//能否改变位置
	DWORD                           m_dwSelectIndex;					//选择索引
...
public:
	...
	//设置弹起扑克
    DWORD SetShootCardData(const BYTE bCardData[], DWORD dwCardCount);
	//获取标记扑克
	BYTE GetMarkCard();
	//标志可进贡扑克
	void MarkCard(BYTE bCard,BYTE bSeries);
...
private:
    ...
    //改变位置
	void ChangePosition(DWORD dwIndex);
...
public:
	//获取控件扑克
	DWORD GetCardData(BYTE  bCardData[], BYTE bCardCount);
