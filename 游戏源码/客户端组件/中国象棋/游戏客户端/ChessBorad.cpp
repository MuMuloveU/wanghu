...
//位置定义
#define EXCURSION_X					0									//偏移坐标
#define EXCURSION_Y					8									//偏移坐标
#define CHESS_WIDTH					47									//棋子宽度
#define CHESS_HEIGHT				47									//棋子高度
#define SPACE_BORAD_X				50									//间隙大小
#define SPACE_BORAD_Y				50									//间隙大小
...
//设置棋盘
void CChessBorad::SetChessBorad(tagChessItem * ChessBorad[9][10])
{
	//辅助变量
	m_cbXPosSelect=255;
	m_cbYPosSelect=255;
...
{ 
	if (m_cbUserColor==WHITE_CHESS) cbXPos=8-cbXPos;
...
{ 
	if (m_cbUserColor==WHITE_CHESS) cbYPos=9-cbYPos;
...
{
	if ((MousePoint.x>EXCURSION_X)&&(MousePoint.x<(EXCURSION_X+9*SPACE_BORAD_X))
		&&(MousePoint.y>EXCURSION_Y)&&(MousePoint.y<(EXCURSION_Y+10*SPACE_BORAD_Y)))
	{
...
void CChessBorad::OnPaint()
{
	...
	for (BYTE cbXPos=0;cbXPos<9;cbXPos++)
	{
		for (BYTE cbYPos=0;cbYPos<10;cbYPos++)
		{
			//计算位置
