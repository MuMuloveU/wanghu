	...
	case GER_NORMAL:		//常规结束
		{
			...
			if (m_wProvideUser!=INVALID_CHAIR)
			{
				...
				{
					m_ChiHuResult[m_wProvideUser].dwChiHuRight|=CHR_ZI_MO;
					if(m_ChiHuResult[m_wProvideUser].dwChiHuRight&CHR_QIANG_GANG)
					{
						m_ChiHuResult[m_wProvideUser].dwChiHuRight &=~CHR_QIANG_GANG;
						m_ChiHuResult[m_wProvideUser].dwChiHuRight |= CHR_GANG_SHANG_HUA;
					}
					//循环累计
					for (WORD i=0;i<GAME_PLAYER;i++)
					{
					...
					//杠上花放杠包牌
					if(m_wProvideUser!=m_wGangProvider)
					{
						//循环累计
						for (WORD i=0;i<m_wPlayerCount;i++)
						{
							//放杠过滤
							if (i == m_wGangProvider) continue;	

							//赢家过滤
							if(i == m_wProvideUser) continue;

							GameEnd.lGameScore[m_wGangProvider]+=GameEnd.lGameScore[i];	
				...
				//抢杠全包
				if((wChairID != m_wProvideUser)&&(m_ChiHuResult[wChairID].dwChiHuRight &CHR_QIANG_GANG))
				{
					//循环累计
					for (WORD i=0;i<m_wPlayerCount;i++)
					{
						...
						GameEnd.lGameScore[m_wProvideUser]+=GameEnd.lGameScore[i];	
						GameEnd.lGameScore[i] = 0L;
					}
				}
				//庄家设置
...
bool CTableFrameSink::OnUserOperateCard(WORD wChairID, BYTE cbOperateCode, BYTE cbOperateCard)
{
	...
	if (m_wCurrentUser==INVALID_CHAIR)
	{
		...
		case WIK_GANG:		//杠牌操作
			{
				...
				if ((m_cbSendCardCount==1)&&(m_cbOutCardData==0))
				{
					...
					{
						if (i==wTargetUser)
							m_lGameScore[i]+=3*m_pGameServiceOption->lCellScore;
						else 
							m_lGameScore[i]-=1*m_pGameServiceOption->lCellScore;
					}
				...
				else
				{
					...
					//杠牌积分
					m_lGameScore[wTargetUser]+=3*m_pGameServiceOption->lCellScore;
					m_lGameScore[m_wProvideUser]-=3*m_pGameServiceOption->lCellScore;
		...
		{
			m_wGangProvider = OperateResult.wProvideUser;
			m_bGangStatus=true;
		}
		...
		case WIK_GANG:			//杠牌操作
			{
				...
				else
				{
					...
					{
						if (i==wChairID) 
							m_lGameScore[i]+=6*m_pGameServiceOption->lCellScore;
						else 
							m_lGameScore[i]-=2*m_pGameServiceOption->lCellScore;
					}
