...
bool CGameClientDlg::OnSubOutCard(const void * pData, WORD wDataSize)
{
	...
	if (m_wCurrentUser!=INVALID_CHAIR)
	{
		if (m_cbTurnCardCount!=0)
		{
			...
			SetGameTimer(m_wCurrentUser,IDI_OUT_CARD,((cbCardCount<=3)&&(cbCardCount<m_cbTurnCardCount))?1:TIME_OUT_CARD);
		}
...
LRESULT CGameClientDlg::OnMessageOutCard(WPARAM wParam, LPARAM lParam)
{
	...
	PlayGameSound(AfxGetInstanceHandle(),(cbCardType>=CT_BOMB)?TEXT("BOMB"):TEXT("OUT_CARD"));
