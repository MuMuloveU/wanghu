...
<%
  ...
  if action="up" then
	 ...
	 set rs=conn.execute("select * from AccountsInfo where Accounts='"& session("UserName") &"' and LogonPass='"& password &"'")
	 if not rs.eof then	 
	 set rs=conn.execute("update AccountsInfo set LogonPass='"& password1 &"' where UserID="& Session("UserID") &"")
	 response.Write("<script language='javascript'>alert('密码更改成功！');location.href='default.asp';</script>")
	 else
  ...
  <tr>
    ...
    <td align="center" valign="top"><table width="585" height="24" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
      ...
      <table width="585" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
        <tr>
          <td>&nbsp;&nbsp;&nbsp;<STRONG class="12_pxcu">&nbsp;修改密码</STRONG></td>
        </tr>
      </table>
      <hr width="580" size="1">
