...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btAddTimes1.SetButtonImage(IDB_BT_ADD_GOLD,hInstance,false);
	m_btAddTimes2.SetButtonImage(IDB_BT_ADD_GOLD,hInstance,false);
	m_btAddTimes3.SetButtonImage(IDB_BT_ADD_GOLD,hInstance,false);

	for( WORD i = 0; i < GAME_PLAYER; i++ )
	{
		m_GoldView[i*2+1].SetGoldIndex(1);
	}
...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	...
	m_GoldStation[1].SetPoint(nXCenterPos+77,nYCenterPos-90);
	m_GoldStation[2].SetPoint(nXCenterPos+89,nYCenterPos-3);
	m_GoldStation[3].SetPoint(nXCenterPos+89,nYCenterPos+65);
	m_GoldStation[4].SetPoint(nXCenterPos,nYCenterPos+82);
	m_GoldStation[5].SetPoint(nXCenterPos-77,nYCenterPos+82);
	m_GoldStation[6].SetPoint(nXCenterPos-88,nYCenterPos-5);
	m_GoldStation[7].SetPoint(nXCenterPos-88,nYCenterPos-71);
	...
	DeferWindowPos(hDwp,m_btFollow,NULL,nXControl+rcButton.Width()+5,nYControl,0,0,uFlags);
	DeferWindowPos(hDwp,m_btShowHand,NULL,nXControl+rcButton.Width()+5,nYControl+rcButton.Height()+5,0,0,uFlags);
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	if (lTotalGold!=0L)
	{
		...
		DrawText(pDC->m_hDC,szBuffer,lstrlen(szBuffer),CRect(nWidth/2-30,nHeight/2-8,nWidth/2+30,nHeight/2+8),uDrawFormat);
	}
...
void CGameClientView::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	for( WORD i = 0; i < CountArray(byCard); i++ )
	{
		for( WORD j = 0; j < GAME_PLAYER; j++ )
			DispatchUserCard( j,byCard[i] );
	}
