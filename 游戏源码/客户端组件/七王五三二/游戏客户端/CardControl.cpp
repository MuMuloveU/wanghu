...
//设置扑克图片
void CCardControl::SetCardImage(UINT nResourceID,bool bTransprant)
{
	if(bTransprant)
		m_bTransprant=true;
	else
	...
	if (m_bLoad==false)
	{
		//加载资源
		m_bLoad=true;
		m_ImageCard.SetLoadInfo(nResourceID,AfxGetInstanceHandle());

		//获取大小
		CImageHandle CardImageHandle(&m_ImageCard);
		m_CardSize.cx=m_ImageCard.GetWidth()/13;
		m_CardSize.cy=m_ImageCard.GetHeight()/4;
	}
...
void CCardControl::OnPaint()
{
	...
	for (INT i=0;i<m_CardDataItem.GetCount();i++)
	{
		...
		if(!m_bTransprant)
			...
			m_ImageCard.AlphaDrawImage(&dc,dwXScreenPos,dwYScreenPos,m_CardSize.cx,m_CardSize.cy,dwXImagePos,dwYImagePos,RGB(255,0,255));
	}
