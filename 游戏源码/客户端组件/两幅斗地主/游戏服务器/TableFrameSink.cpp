	...
	case GER_USER_LEFT:		//用户强退
		{
			...
			for (WORD i=0;i<m_wPlayerCount;i++)
			{
				...
				if (i==wChairID)
				{
					...
					lScore	 =-m_wBombTime*m_pGameServiceOption->lCellScore*4;
					GameEnd.lGameScore[i]=-m_wBombTime*m_pGameServiceOption->lCellScore*4;
				}
				else if (m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
				{
					//统计积分
					ScoreKind=enScoreKind_Win;
					lScore=(m_wBombTime*m_pGameServiceOption->lCellScore*4)/3;
					GameEnd.lGameScore[i]=(m_wBombTime*m_pGameServiceOption->lCellScore*4)/3;
