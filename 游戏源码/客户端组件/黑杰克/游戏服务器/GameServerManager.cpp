...
CGameServiceManager::CGameServiceManager(void)
{
	...
	//m_GameServiceAttrib.dwMaxClientVersion	= MAXCLIENTVER;
	//m_GameServiceAttrib.dwLessClientVersion	= LESSCLIENTVER;
	...
	lstrcpyn(m_GameServiceAttrib.szKindName,
		     GAME_NAME,
			 CountArray(m_GameServiceAttrib.szKindName));

	lstrcpyn(m_GameServiceAttrib.szDataBaseName,
             szTreasureDB,
			 CountArray(m_GameServiceAttrib.szDataBaseName));

	lstrcpyn(m_GameServiceAttrib.szDescription,
             TEXT("21点游戏服务组件"),
			 CountArray(m_GameServiceAttrib.szDescription));

	lstrcpyn(m_GameServiceAttrib.szServerModuleName,
		     TEXT("BJServer.dll"),
			 CountArray(m_GameServiceAttrib.szServerModuleName));
 
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,
			 TEXT("BlackJack.exe"),
			 CountArray(m_GameServiceAttrib.szClientModuleName));
