...
//					//else 
//						if (cbBombCardType == CT_MISSILE_CARD) cbTmpSingleCount += 5 ;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//函数功能：在数组中找到指定的牌
//函数参数: cards为一副牌，size为这副牌的牌数，card为要找的牌(一张)
//函数返回:
int	CGameLogic::FindCard(unsigned char cards[],unsigned char size,const unsigned char &card)
{
	int i;
	for (i=0; i<size; i++)
	{
...
//是否有A44
bool CGameLogic:: A44(BYTE iCardList[], int iCardCount)
{
	int Number4 = 0;
	int NumberA = 0;
	for(int i = 0; i < iCardCount; i++)
...
//是否有双王
bool CGameLogic:: DoubleKong(BYTE iCardList[], int iCardCount)
{
	int temp = 0;
	for(int i = 0; i < iCardCount; i++)
	{
		if(iCardList[i] == 0x4E || iCardList[i] == 0x4F) temp++;
	}
	if( temp >= 2 )
	{
		return TRUE;
...
//函数参数: cards为一副牌，size为这副牌的牌数，card为要找的牌(一张)
//函数返回:
int	CGameLogic::FindCardNum(unsigned char cards[],unsigned char size,const unsigned char &card)
{
	 int num = 0;
	for (int i=0; i<size; i++)
	{
		if (GetCardValue(card) == GetCardValue(cards[i]))
		{
			 num++;
		}
	}
