...
struct tagAnalyseResult
{
	...
	tagSameDataInfo					SameDataInfo[PACK_COUNT];			//同牌数据
	tagTractorDataInfo				TractorDataInfo[PACK_COUNT-1];		//拖牌数据
};

//需求结构
struct tagDemandInfo
{
	BYTE 							cbMaxTractor[PACK_COUNT-1];			//长度需求
	BYTE							cbSameCardCount[PACK_COUNT];			//同牌需求
	BYTE 							cbTractorCardCount[PACK_COUNT-1];		//拖牌需求
};
