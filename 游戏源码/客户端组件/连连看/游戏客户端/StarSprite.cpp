...
void CStarSprite::SetRunPos(CPoint src, CPoint des)
{
	m_SrcPoint = src; 
	m_DesPoint = des;
}
...
// 初始
void CStarSprite::zeroSpriteValues(void)
{
	m_bActive = FALSE;
	m_bFirstFrame = FALSE;
}

void CStarSprite::DrawPointSprite(void)
{
	if(!m_bActive)
		return;
	D3DXMATRIX matTrans;
...
// 设置顶点精灵参数
void CStarSprite::SetPointSprite(LPDIRECT3DDEVICE9		pDevice, LONG RCID)
{
	m_pDevice = pDevice;
	initPointSprites( RCID );
}

void CStarSprite::initPointSprites( LONG RCID )
{
	//
...
void CStarSprite::renderPointSprites( void )
{
	//
