...
CGameServiceManager::CGameServiceManager(void)
{
	...
	lstrcpyn(m_GameServiceAttrib.szDataBaseName,TEXT("WHSanDaYiDB"),CountArray(m_GameServiceAttrib.szDataBaseName));
	lstrcpyn(m_GameServiceAttrib.szDescription,TEXT("常州三打一游戏服务组件"),CountArray(m_GameServiceAttrib.szDescription));
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,TEXT("SanDaYi.EXE"),CountArray(m_GameServiceAttrib.szClientModuleName));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("SanDaYiServer.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));

#ifdef VIDEO_GAME
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,TEXT("SanDaYiVD.EXE"),CountArray(m_GameServiceAttrib.szClientModuleName));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("SanDaYiVDServer.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));
#endif
	return;
}
...
bool __cdecl CGameServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption)
{
	...
	{
		pGameServiceOption->lLessScore=__max(pGameServiceOption->lCellScore*18L,pGameServiceOption->lLessScore);
