...
VOID CGameClientView::RectifyGameView(INT nWidth, INT nHeight)
{
	...
	DeferWindowPos(hDwp,m_btSortCardOrder,NULL,nWidth-rcControl.Width()-5,nHeight-rcControl.Height()*4-20,0,0,uFlags);
...
VOID CGameClientView::DrawGameView(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	_sntprintf(szBuffer,CountArray(szBuffer),TEXT("本轮 %d 分"),m_TurnScore);
	pDC->TextOut(75,80,szBuffer,lstrlen(szBuffer));
...
{
	//发送消息
	AfxGetMainWnd()->SendMessage(IDM_SORT_HAND_CARD,ST_COLOR,ST_COLOR);
...
{
	//发送消息
	AfxGetMainWnd()->SendMessage(IDM_SORT_HAND_CARD,ST_COUNT,ST_COUNT);
...
{
	//用户扑克
	m_HandCardControl[1].SetBenchmarkPos(175,nHeight/2-50,enXLeft,enYCenter);
	m_HandCardControl[3].SetBenchmarkPos(nWidth-175,nHeight/2-50,enXRight,enYCenter);

	//出牌扑克
	m_UserCardControl[1].SetBenchmarkPos(nWidth*3/10,nHeight/2-65,enXCenter,enYCenter);
	m_UserCardControl[3].SetBenchmarkPos(nWidth*7/10,nHeight/2-65,enXCenter,enYCenter);
	...
	DeferWindowPos(hDwp,m_DlgVideoService[3],NULL,nWidth-m_nXBorder-5-rcAVDlg.Width(),nHeight/2+30,0,0,uFlags);
	DeferWindowPos(hDwp,m_DlgVideoService[1],NULL,m_nXBorder+5,nHeight/2+30,0,0,uFlags);
	...
	DeferWindowPos(hDwp,m_DlgVideoService[2],NULL,m_nXBorder+5,nHeight-m_nYBorder-53-rcAVDlg.Height(),0,0,uFlags);
